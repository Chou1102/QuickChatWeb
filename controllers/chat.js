...
const deleteChat = async (req, res) => {
  ...
  const isUserInChat = chat.users.includes(req.user.id);
  if (!isUserInChat) {
    throw new BadRequestError("You are not authorized to delete this chat");
  }

  // For group chats, only the admin can delete
  if (chat.isGroupChat && chat.groupAdmin.toString() !== req.user.id) {
    throw new BadRequestError("Only group admin can delete the chat");
  }
  ...
  res.status(StatusCodes.OK).json({ message: "Chat deleted successfully" });
};
